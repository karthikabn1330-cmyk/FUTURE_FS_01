        ...
        <div class="max-w-screen-xl mx-auto px-4 py-10 animate-fade-in">
             <div class="flex justify-between items-end mb-8 border-b border-white/10 pb-6">
                 ...
                 <div class="flex gap-3">
                     <button onclick="resetDatabase()" class="px-4 py-2 border border-red-500/50 text-red-400 text-sm rounded hover:bg-red-500 hover:text-white transition-colors">Reset Data</button>
                     <button onclick="addNewShowroomPrompt()" class="px-4 py-2 bg-gold text-black font-bold text-sm rounded hover:bg-white">+ Add Showroom</button>
                 </div>
             </div>
             ...
             <div class="grid gap-6 md:grid-cols-2">
                 <div>
                     ...
                     <div class="space-y-4">
                         ${myShowrooms.map(sr => {
        const inventory = state.products.filter(p => p.showroomId === sr.id).length;
        return `
...
function addNewShowroomPrompt() {
    const name = prompt("Showroom Name:");
    if (!name) return;
